...
public class HeartBeatShow extends AppCompatActivity {
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.heart_beat_show);
        ...
        TextView text_name = findViewById(R.id.heart_beat_name);
        ...
        btn_delete.setOnClickListener(v -> {
            ...
            HttpDelete httpDelete = new HttpDelete("https://app-saude-unisc.herokuapp.com/api/v1/heartbeat/" + myIntent.getStringExtra("id"));
            httpDelete.setHeader("Authorization", "Bearer " + access_code);
            Thread thread = new Thread(() -> {
                try {
                    HttpResponse response = httpclient.execute(httpDelete);
                    if (response.getStatusLine().getStatusCode() == 200) {
                        Intent appInfo = new Intent(HeartBeatShow.this, HeartBeat.class);
                        startActivity(appInfo);
                    } else {
                        runOnUiThread(() -> {
                            final Toast toast = Toast.makeText(HeartBeatShow.this,
